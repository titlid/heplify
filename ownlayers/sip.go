...
func (s *SIP) DecodeFromBytes(data []byte, df gopacket.DecodeFeedback) error {
	...
	for {
		...
		// log.Println("llllllllllllll", string(line))
		...
		if countLines == 0 {
			...
			if !strings.Contains(string(line), "HEP3") {
...
func (s *SIP) GetFirstLine() string {
	return string(s.FirstLine)
}
